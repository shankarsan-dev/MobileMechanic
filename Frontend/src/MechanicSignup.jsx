...
const MechanicSignup = () => {
  ...
  const validate = () => {
    ...
    tempErrors.gender = gender ? '' : 'Gender is required.';
    tempErrors.address = address ? '' : 'Address is required.';
    tempErrors.idDocument = idDocument ? '' : 'ID Document is required.'; // Validation for ID document
    tempErrors.photo = photo ? '' : 'Photo is required.'; // Validation for photo
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      ...
      formData.append('idDocument', idDocument); // Append ID document file
      formData.append('photo', photo); // Append photo file

      axios.post(`${server}/api/mechanics/signup`, formData, {
        headers: {
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full p-2 border border-gray-300 rounded mt-1"
            />
            {errors.idDocument && <p className="text-red-600 text-sm mt-1">{errors.idDocument}</p>}
          </div>
          ...
          {errors.apiError && <p className="text-red-600 text-sm mt-4">{errors.apiError}</p>}
          {success && <p className="text-green-600 text-sm mt-4">{success}</p>}
        </form>

        <div className="mt-6 text-center">
          <p>Already have an account? <Link to="/mechanic-login" className="text-blue-600">Log in</Link></p>
        </div>
      </div>
    </div>
  );
