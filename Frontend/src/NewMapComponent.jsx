...
//   const previousStatus = useRef(null); // Ref to track previous status for changes
// console.log("mechanicid " + mechanicId)
//   useEffect(() => {
//     const fetchData = async () => {
//       try {
...
//         setMechanicLocation(mechanicData);
//         setCustomerLocation(customerData);
//         setMechanicDetails(serviceData.mechanicId); // Assume mechanic details are inside serviceData
...
//   useEffect(() => {
//     // Listen for service status updates via the socket
//     socket.on('serviceStatusUpdate', ({ status, serviceId }) => {
//       // Update the service details with the new status
//       setServiceDetails((prevDetails) => ({
//         ...prevDetails,
//         status
//       }));
...
//         // Trigger a notification with the updated status
//         setNotification({
//           title: 'Service Status Update',
//           message: `The service status has been updated to: ${status}.`,
//           serviceId // You can use this to link to more details if needed
//         });
//         previousStatus.current = status; // Update the previous status
//       }
//     });
...
//     setNotification(null); // Clear the notification state
//   };
...
//       // Check if the status has changed
//       if (previousStatus.current !== status) {
//         // Scroll to the top before showing the notification
//         window.scrollTo({ top: 0, behavior: 'smooth' });

//         // Trigger a notification with the updated status after a brief timeout to ensure scroll completes
//         setTimeout(() => {
//           setNotification({
//             title: 'Service Status Update',
//             message: `The Mechanic is: ${status}.`,
//             serviceId // You can use this to link to more details if needed
//           });
...
const NewMapComponent = ({ serviceId, customerId, mechanicId, socket }) => {
  ...
  //   try {
  //     await axios.post(`http://localhost:5000/api/mechanic/rate`, { rating });
  ...
  const handleRatingSubmit = async (rating, comment) => {
    try {
      await axios.post(`http://localhost:5000/api/mechanic/rate`, {
        mechanicId, // Ensure this variable is defined in your component
        customerId, // Ensure this variable is defined in your component
        rating, // Optional, only if you want to include a comment
      });
      // Optionally, show a success notification or update UI after rating submission
      setNotification({
        title: 'Rating Submitted',
