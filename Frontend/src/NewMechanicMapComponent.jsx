...
const NewMechanicMapComponent = ({ serviceId, mechanicId, customerId, socket }) => {
  ...
  // Listen for new ratings from the socket
  useEffect(() => {
    socket.on('newRating', (rating) => {
      setRatings((prevRatings) => [...prevRatings, rating]); // Update the ratings state
      console.log('New rating received:', rating); // Optional: Log the new rating
    });

    return () => {
      socket.off('newRating'); // Cleanup on unmount
    };
  }, [socket]);
