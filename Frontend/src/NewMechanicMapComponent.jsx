...
const NewMechanicMapComponent = ({ serviceId, mechanicId, customerId, socket }) => {
  ...
  const [path, setPath] = useState([]);
  const [notification, setNotification] = useState(null); // State for notification
  const [clickedStatus, setClickedStatus] = useState(null); // Track clicked button
  const notificationRef = useRef(null); // Ref for the notification component
  ...
  const updateServiceStatus = async (status) => {
    try {
      ...
      setClickedStatus(status); // Set the clicked status
  ...
  return (
    <>
      ...
      <div style={{ marginTop: '20px' }} className="flex space-x-2">
        <button
          onClick={() => updateServiceStatus('on the way')}
          className={`px-4 py-2 rounded ${clickedStatus === 'on the way' ? 'bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'} text-white`}
          disabled={clickedStatus === 'on the way'}
        >
          On the Way
        </button>
        <button
          onClick={() => updateServiceStatus('arrived')}
          className={`px-4 py-2 rounded ${clickedStatus === 'arrived' ? 'bg-green-700' : 'bg-green-500 hover:bg-green-600'} text-white`}
          disabled={clickedStatus === 'arrived'}
        >
          Arrived
        </button>
        <button
          onClick={() => updateServiceStatus('completed')}
          className={`px-4 py-2 rounded ${clickedStatus === 'completed' ? 'bg-yellow-700' : 'bg-yellow-500 hover:bg-yellow-600'} text-white`}
          disabled={clickedStatus === 'completed'}
        >
          Completed
        </button>
      </div>
