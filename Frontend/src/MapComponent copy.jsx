...
const MapComponent = ({ socket, latitude, longitude, mechanics, vehicleType, description, radius = 5 }) => {
  ...
  useEffect(() => {
    if (latitude && longitude && customerId) {
      updateLocationInDatabase(customerId, latitude, longitude);
    }
  ...
  // Add customer marker on the map
  useEffect(() => {
    if (mapRef.current && !customerMarkerRef.current && customerId) {
      customerMarkerRef.current = L.marker(position, {
        icon: customerIcon,
        draggable: true,
      })
  ...
  useEffect(() => {
    if (selectedMechanic) {
      // Fetch the average rating for the selected mechanic
      const fetchAverageRating = async () => {
        try {
          const response = await axios.get(
            `http://localhost:5000/api/mechanics/${selectedMechanic._id}/average-rating`
          );
