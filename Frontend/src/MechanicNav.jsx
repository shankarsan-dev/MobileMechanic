...
const MechanicNav = ({ socket, mechanicId }) => {
  const navigate = useNavigate();

  const handleLogoutClick = () => {
    if (mechanicId) {
      // Emit the event to set mechanic as unavailable before logging out
      console.log('Setting mechanic as unavailable due to logout');
      socket.emit('MsetUnavailable', mechanicId);
    }

    // Proceed with clearing token and navigating after the event
    localStorage.removeItem('token');
    navigate('/');
  };
