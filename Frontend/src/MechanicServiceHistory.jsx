...
const CustomerServiceHistory = () => {
  ...
  const fetchServices = async () => {
    try {
      ...
      const mechanicId = decodedToken.id; // Extract customerId from the token
      console.log("customerid: "+mechanicId);
      const response = await axios.get(`${server}/api/mechanics/services/${mechanicId}`, {
          ...
          {services.map((service) => (
            <tr key={service._id}>
              ...
              <td className="py-2 px-4 border-b">
                {service.customerId ? (
                  <>
                    {service.customerId.phoneNumber}
                  </>
                ) : (
                  'Not Assigned' // Fallback text when no mechanic is assigned
                )}
